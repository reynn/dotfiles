...
options:
  ...
  state:
    description:
      - The state of module
    type: str
    choices: [ absent, force, latest, present ]
    default: present
  extra_args:
    description:
  ...
  executable:
    description:
      - The explicit executable or pathname for the cargo executable,
...
EXAMPLES = '''
# Install (exa) Rust package.
- cargo:
    name: exa

# Install (exa) Rust package on version 0.11.
- pip:
    name: exa==0.11

# Install (exa) Rust package with version specifiers
- pip:
    name: exa>0.10,<0.20,!=0.11
'''
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            crate_path=dict(type='str'),
            registry=dict(type=''),
            extra_args=dict(type='str'),
        ),
        required_one_of=[['name', 'crate_path']],
        mutually_exclusive=[['name', 'crate_path'], ['executable', 'virtualenv']],
