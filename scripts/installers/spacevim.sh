...
install_neovim() {
  ...
  if [[ -d "${nvim_path}" ]]; then
    if [[ "$(readlink ${nvim_path})" =~ \.SpaceVim$ ]]; then
      success "Installed SpaceVim for neovim"
    else
      mv "${nvim_path}" "${nvim_path}_back"
      success "BackUp ${nvim_path} to ${nvim_path}_back"
      ln -s "$HOME/.SpaceVim" "${nvim_path}"
      success "Installed SpaceVim for neovim"
    fi
  else
    mkdir -p "$HOME/.config"
...
uninstall_neovim() {
  ...
  if [[ -d "${nvim_path}" ]]; then
    if [[ "$(readlink ${nvim_path})" =~ \.SpaceVim$ ]]; then
      ...
      if [[ -d "${nvim_path}_back" ]]; then
        mv "${nvim_path}_back" "${nvim_path}"
        success "Recover from ${nvim_path}_back"
      fi
    fi
  fi
}
