...
function __aws_ec2_instance_json_generate -d "Create the necessary JSON data to send to AWS via the CLI" -a "AMIID"
    set -l ami_id ''
    ...
    getopts $argv | while read -l key value
        switch $key
            ...
            case c launch_count
                set -x launch_count "$value"
            case r role_type
                set -x role_type "$value"
            case n name
                set -x name "$value"
            case s security_group
                set -x security_group "$value"
            case subnet_id
                set -x subnet_id "$value"
            case v verbose
                set -x DEBUG 'true'
        end
    end
    ...
    if test -z $security_group
        ...
        log.debug "Selected security group $security_group"
        if test -z "$security_group"
            log.error "No security group selected"
            return 1
        end
    end
    if test -z $subnet_id
        ...
        log.debug "SecurityGroup: $security_group is in VPC $vpc_id looking for matching subnets"
        ...
        log.debug "Selected subnet $subnet_id"
    end

    log.debug "AMI_ID         $ami_id"
    log.debug "INSTANCE_TYPE  $instance_type"
    log.debug "COUNT          $launch_count"
    log.debug "ROLE_TYPE      $role_type"
    log.debug "SECURITY_GROUP $security_group"
    log.debug "SUBNET_ID      $subnet_id"
