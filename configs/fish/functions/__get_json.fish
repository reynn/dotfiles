...
function __get_json --description ''
    ...
    function __check_cache
        ...
        test -n (command -v wget); and wget -q -O "$cache_path" "$url"; or curl -sSLs "$url" -o "$cache_path"
        echo $cache_path
    end

    log.debug "local_cache_dir  : $local_cache_dir"
    log.debug "cache_timeout    : $cache_timeout"
    log.debug "urls             : $urls"
