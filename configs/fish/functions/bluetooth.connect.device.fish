function bluetooth.connect.device -d 'Connect to a Bluetooth device'
    ...
    if not utils.command.available -c 'osascript'
        log.error -m '`osascript` is not installed'
        return 1
    end
    ...
    if test -z "$device_address"
        log.debug -m 'No device selected'
        return 0
    end
    log.info -l 'bluetooth' -m "Connecting to device $device_name [$device_address]"
