...
function vault.login.k8s -d 'Login to a vault instance using a Kubernetes service account'
    ...
    if set -q K8S_NAMESPACE
        set -x k8s_secret_name (kubectl get sa -n $K8S_NAMESPACE $K8S_SA -o json | dasel -r json -w plain '.secrets.first().name')
        set -x k8s_sa_token (kubectl get secret -n $K8S_NAMESPACE $k8s_secret_name -o json | dasel -r json -w plain .data.token | base64 -d)
    else
        set -x k8s_secret_name (kubectl get sa $K8S_SA -o json | dasel -r json -w plain '.secrets.first().name')
        set -x k8s_sa_token (kubectl get secret $k8s_secret_name -o json | dasel -r json -w plain .data.token | base64 -d)
    end
